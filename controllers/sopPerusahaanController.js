// controllers/sopPerusahaanController.js

const db = require('../db');
const path = require('path');
const fs = require('fs');

// === Upload SOP Perusahaan ===
exports.upload = (req, res) => {
  ...
  db.query(sql, values, (err) => {
    ...
    res.status(200).json({ message: 'SOP berhasil diunggah.' });
  });
};
...
exports.getAll = (req, res) => {
  const sql = 'SELECT * FROM sop_perusahaan ORDER BY tanggal DESC';
...
exports.search = (req, res) => {
  ...
  const sql = `SELECT * FROM sop_perusahaan WHERE title LIKE ? OR keterangan LIKE ?`;
...
exports.filterByTanggal = (req, res) => {
  ...
  const sql = `SELECT * FROM sop_perusahaan WHERE tanggal BETWEEN ? AND ?`;
...
exports.update = (req, res) => {
  ...
  if (file) {
    db.query('SELECT file FROM sop_perusahaan WHERE id = ?', [id], (err, results) => {
      if (err || results.length === 0) return res.status(404).json({ message: 'Data tidak ditemukan.' });

      const oldFile = results[0].file;
      const filePath = path.join(__dirname, '../uploads/sop/', oldFile);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);

      const sqlUpdate = `
        UPDATE sop_perusahaan SET title=?, tanggal=?, keterangan=?, revisi=?, file=? WHERE id=?
      `;
      db.query(sqlUpdate, [title, tanggal, keterangan, revisi, file.filename, id], (err) => {
        ...
        res.status(200).json({ message: 'SOP berhasil diupdate.' });
      });
    });
  } else {
    const sql = `UPDATE sop_perusahaan SET title=?, tanggal=?, keterangan=?, revisi=? WHERE id=?`;
...
exports.remove = (req, res) => {
  ...
  db.query('SELECT file FROM sop_perusahaan WHERE id = ?', [id], (err, results) => {
    ...
    if (filename) {
      const filePath = path.join(__dirname, '../uploads/sop-perusahaan', filename);
    ...
    db.query('DELETE FROM sop_perusahaan WHERE id = ?', [id], (err) => {
      if (err) return res.status(500).json({ message: 'Gagal menghapus dokumen.', error: err });
      res.status(200).json({ message: 'SOP berhasil dihapus.' });
    });
  });
};
...
exports.updateStatus = (req, res) => {
  ...
  const sql = `UPDATE sop_perusahaan SET status=?, alasan_reject=? WHERE id=?`;
