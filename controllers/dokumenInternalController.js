...
// === Ambil Semua Dokumen Internal ===
exports.getAll = (req, res) => {
  const sql = 'SELECT * FROM dokumen_internal ORDER BY tanggal DESC';
...
exports.search = (req, res) => {
  ...
  const sql = `SELECT * FROM dokumen_internal WHERE title LIKE ? OR keterangan LIKE ?`;
...
exports.filterByTanggal = (req, res) => {
  ...
  const sql = `SELECT * FROM dokumen_internal WHERE tanggal BETWEEN ? AND ?`;
...
exports.update = (req, res) => {
  ...
  if (file) {
    db.query('SELECT file FROM dokumen_internal WHERE id = ?', [id], (err, results) => {
      if (err || results.length === 0) return res.status(404).json({ message: 'Data tidak ditemukan.' });

      const oldFile = results[0].file;
      const filePath = path.join(__dirname, '../uploads/internal/', oldFile);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);

      const sqlUpdate = `
        UPDATE dokumen_internal SET title=?, tanggal=?, keterangan=?, revisi=?, file=? WHERE id=?
      `;
    ...
    const sql = `UPDATE dokumen_internal SET title=?, tanggal=?, keterangan=?, revisi=? WHERE id=?`;
...
exports.remove = (req, res) => {
  ...
  db.query('SELECT file FROM dokumen_internal WHERE id = ?', [id], (err, results) => {
    ...
    const filePath = path.join(__dirname, '../uploads/internal/', results[0].file);
    if (fs.existsSync(filePath)) fs.unlinkSync(filePath);

    db.query('DELETE FROM dokumen_internal WHERE id = ?', [id], (err) => {
...
exports.updateStatus = (req, res) => {
  ...
  const sql = `UPDATE dokumen_internal SET status=?, alasan_reject=? WHERE id=?`;
